...
class ScopeRequest:
    """Advisory scope-expansion evidence emitted by Claude."""
...
def parse_scope_request(final_message: str) -> ScopeRequest | None:
    """Parse the strict advisory ``META SCOPE REQUEST v1`` block.

    A malformed block is deliberately indistinguishable from no request to
    callers that make authority decisions.  The orchestrator may separately
    record that the marker was present so operators can see the warning.
    """
...
REVISION_TOOLS = "Read,Edit,Write,Grep,Glob"
